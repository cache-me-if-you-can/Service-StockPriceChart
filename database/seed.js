...
const sampleStockPrice = () => {
  ...
  for (let hour = 9; hour < 18; hour++) {
    for (let minute = 0; minute <= 60; minute+=5) {
      ...
      result.push({ "symbol": symbol, "name": "Merry Men", "price": randomPrice, "owner": 4, "rating": 4, "date": d });
    }
  }
...
const sampleStockPrice2 = () => {
  ...
  for (let hour = 9; hour < 18; hour++) {
    for (let minute = 0; minute <= 60; minute += 5) {
      ...
      const randomPrice = Number(faker.finance.amount(10, 100, 2));
      const d = new Date(2018, 9, 25, hour, minute);
      result.push({ "symbol": symbol, "name": companyName, "price": randomPrice, "owner": randomOwner, "rating": randomOwner, "date": d });
    }
  }
